...
local lang = redis:get(hash)
local data = load_data(_config.moderation.data)
if tonumber(msg.from.id) == SUDO and is_GolPesareIran(msg) then
  ...
  if matches[2] and not string.match(matches[2], '^%d+$') then
   tdcli_function ({
	...
	if ((matches[1]:lower() == "sendfile" ) or (matches[1] == 'ارسال فایل' )) and matches[2] and matches[3] then
		...
		tdcli.sendDocument(msg.chat_id_, msg.id_,0, 1, nil, send_file, SuperZedlinkch, dl_cb, nil)
	end
	if ((matches[1]:lower() == "sendplug" ) or (matches[1] == 'ارسال پلاگین' )) and matches[2] then
	    local plug = "./plugins/"..matches[2]..".lua"
		tdcli.sendDocument(msg.chat_id_, msg.id_,0, 1, nil, plug, SuperZedlinkch, dl_cb, nil)
    end
...
redis:set("SuperZedlinkLikes",0)
redis:set("SuperZedlinkDisLikes",0)
return "انجام شد"
end
if msg.to.type ~= 'pv' then
...
if (matches[1]:lower() == 'SuperZedlink' or matches[1] == 'ماتادور') and is_GolPesareIran(msg) then
local info_text = 
[[
》SuperZedlink Cli v7.5

An advanced administration bot based on https://valtman.name/telegram-cli

》Admins :
》@GolPesareIran ➣ Founder & Developer《
...
Info :]]..check_markdown(SuperZedlinkby)..[[➣]]..check_markdown(SuperZedlinkch)..[[
]]
