...
----------------------------------------
local function delmsg (SuperZedlink,GolPesareIran)
    msgs = SuperZedlink.msgs 
    for k,v in pairs(GolPesareIran.messages_) do
        msgs = msgs - 1
        tdcli.deleteMessages(v.chat_id_,{[0] = v.id_}, dl_cb, cmd)
        if msgs == 1 then
            tdcli.deleteMessages(GolPesareIran.messages_[0].chat_id_,{[0] = GolPesareIran.messages_[0].id_}, dl_cb, cmd)
            return false
        end
    end
    tdcli.getChatHistory(GolPesareIran.messages_[0].chat_id_, GolPesareIran.messages_[0].id_,0 , 100, delmsg, {msgs=msgs})
end
----------------------------------------
...
if ((matches[1]:lower() == 'rmsg' ) or (matches[1] == 'پاکسازی' )) and is_mod(msg) then
	...
	--------------------------------
if ((matches[1]:lower() == "dev" ) or (matches[1] == "برنامه نویس" )) and is_GolPesareIran(msg) then 
      tdcli.sendContact(msg.chat_id_, msg.id_, 0, 1, nil, 989015858478, 'MahDi', 'Mohseni', 782187490)
end
--------------------------------
